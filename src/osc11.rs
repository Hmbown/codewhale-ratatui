...
//!
//! Extracted from the Codewhale engine's `crates/palette/src/osc11.rs`
//! (`Hmbown/CodeWhale` `58b1dd3dd`), including the type-ahead carry from
//! #5925: bytes the user typed while the probe was reading are handed back,
//! never dropped. Hosts replay them with [`take_carried_type_ahead`].
...
/// background query and a host's graphics probes, under the
...
/// CSI-terminated variant of [`query_terminal`] for a primary-DA probe
/// (sixel detection): a primary-DA reply ends at its alphabetic final byte
...
// other byte here; the host drains this and replays it into its input. The buffers live in this module rather than with the replay
// logic so any host can drain them without depending on its event loop.
// ---------------------------------------------------------------------------
